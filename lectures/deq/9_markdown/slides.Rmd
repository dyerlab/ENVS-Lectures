...
## 15 Minute Activity - Make a Quick Report

Let's create a **different** kind of document this time.  Create a new RMarkdown document (use the HTML template for simplicity) and save it as `markdown_test.Rmd` in the same directory as the project.  In this document craft a quick narrative about the data from `Field_Data.csv` and `GIS_Data.csv`.  Make sure to provide an example of at least one each of the following items:

1. Graphical Output

2. A Table of results.

3. In-text interpretaion of R output.

4. A variety of appropriate typesetting elements (Headings, bold, italic, etc.).
