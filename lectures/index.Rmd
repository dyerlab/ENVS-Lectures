...
<div class="nav">
<div class="nav-left">
<a href="https://dyerlab.github.io/DLabWebsite/">The Dyer Laboratory</a>
</div>
<div class="nav-right">
<a href="https://dyerlab.github.io/DLabWebsite/people.html">People</a> &nbsp; 
<a href="https://dyerlab.github.io/DLabWebsite/manuscripts.html">Papers</a> &nbsp; 
<a href="https://dyerlab.github.io/DLabWebsite/software.html">Software</a> &nbsp; 
<a href="https://dyerlab.github.io/ENVS-Lectures/">Teaching</a> &nbsp; 
</div>
</div>
...
nonunderscorification <- function( files, drop_numbers = FALSE ) {
  lastPart <- matrix(unlist(str_split( files, "/")),ncol=2, byrow=TRUE)[,2]
  ret <- unlist( map( str_replace( str_replace_all(lastPart,"_", " "), "./", "" ),
                       gtools::capwords ) )
  if( drop_numbers ) { 
    ret <- str_remove(ret, "^\\d ")
  }
  return(ret)
}
...
makeTable <- function( files, caption = "", drop_numbers=FALSE ) {
  buttons <- numeric(0)
  for( path in files ) {
  ...
  titles <- nonunderscorification( files, drop_numbers )
...
```{r}
files <- list.dirs( path = "deq", recursive = FALSE)
makeTable(files, 
          caption="A series of presentations (and included activities) for a 2-day Introduction to R workshop for new users focusing on tidyverse.",
          drop_numbers = TRUE)
```
