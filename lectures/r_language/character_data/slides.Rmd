...
.footnote[<sup>1</sup>The ellipsis is for additional stuff to be added if present.]
...
This is because the digit indication for a regular expression includes a `\` not as a "escape character" but as part of it directly!  So, we need to escape it as well (confused yet?).
...
This may be convienent when there are a few different kinds of spellings.
...
Or from the contents of an individual data file

```{r}
beerStyles <- readLines( "../../../data/Beer_Styles.csv")
beerStyles[1:10]
```
...
Now you know a few of the rules, lets apply them to a real world problem.  

### Use Case

You are doing some meta-work for the introduction to your masters thesis.  You have a bunch of manuscripts (say 100 of them) that you have identified as having data in them that you would like to use in your meta analysis.  You could:

**Brue Force It:** Read 100 manuscripts and manually extract the data from each one, thereby using what Dyer calls the "Brute Force" method.  If every manuscript is 20 pages long, how long would it take to get through 2000 pages and extract all the data without error?

--

**Use R:** Automate the extraction of the text in the data and search for specific components you are interested in (then enjoy a frosty `r emo::ji("beer")` while it runs and does it for you).  
