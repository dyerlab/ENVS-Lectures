...
rice$Month <- fct_drop( rice$Month )
summary( rice$Month )
```

```{r}
df <- tibble( `Sample Month` = levels( rice$Month ))
df$`Water Temperature (°C)` <- by( rice$H2O_TempC, rice$Month, mean, na.rm=TRUE )
df$`Salinity (ppt)` <- by( rice$Salinity_ppt, rice$Month, mean, na.rm=TRUE)
df$pH <- by( rice$PH, rice$Month, mean, na.rm=TRUE )
df
```
...
library( knitr )
library( kableExtra )
tbl <- kable( df, caption = "Mean values for water temperature, salinity, and pH taken in the first three months of 2014 at the Rice Rivers Center.")
tbl <- kable_minimal(  tbl,  lightable_options = c("striped", "hover"))
tbl <- add_header_above( tbl,  c(" " = 1, "Water Parameter" = 3))
tbl
```
