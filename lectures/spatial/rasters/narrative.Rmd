...
We can also proceed by relying upon the `data.frame` objects representing the elevation.  So let's go back to our the `alt.df` object and use that in combination with a `filter` and plot both `data.frame` objects (the outline of the landscape in gray and the elevation range as a gradient.)
...
alt.df %>%
  ...
  ggplot( aes( x = Longitude,
               y = Latitude) ) + 
  geom_raster( fill = "gray80", data=alt.df ) + 
