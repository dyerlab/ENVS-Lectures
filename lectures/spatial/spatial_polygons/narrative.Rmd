...
We can use `R` to download and unzip the file *in the current data directory* (n.b., you can do it using a browser as well).

To use `R` you need to first download them (I've set `eval=FALSE` to the chuck so it is not redownloaded each time.  Run it by hand using `CTRL/CMD + Return`).
...
Next, we can unzip them in the current directory.

```{r eval=FALSE}
unzip( "./Districts.zip")
```

Now, go look at your directory and you will see a nice set of files named `Zoning_Districts.*`. The `*.shp` files are the actual *shapefiles* and the other stuff are supportive documentation (such as the projection in the .prj file).

```{r}
system("ls -al Zoning*")
```

To load it in, we read the shapefile from the local directory.
...
[^1]: **Dyer's First Law: Reproject then forget**.
