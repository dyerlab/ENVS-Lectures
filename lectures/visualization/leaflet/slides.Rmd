...
```{r eval=FALSE}
addProviderTiles( providerName )
```

where `providerName` can be any of the items found [here](http://leaflet-extras.github.io/leaflet-providers/preview/index.html)  .redinline[  <-- click on this]
...
In addition to the symbology, we can define a wide range of conntent to be associated with each marker.
...
<!-- CHARIS: I'm going to switch to pairs of slides for the very long ones -->
...
<!-- 
    I pulled out the data to make it a bit more explicit for them, it is the first time
...
leaflet(beetles) %>%
  addProviderTiles( providers$CartoDB.Positron ) %>%
  addCircleMarkers( fillColor = "lightblue", 
                    fillOpacity = 1, 
                    color = "blue",
                    weight = 1,
                    group = "beetles",
